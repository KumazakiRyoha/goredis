	...
	for {
		...
		if !state.readingMultiLine {
			...
			if state.finished() {
				...
				if state.msgType == '*' {
					result = reply.MakeMultiBulReply(state.args)
				} else if state.msgType == '$' {
					result = reply.MakeBulReply(state.args[0])
				}
				ch <- &Payload{
					Data: result,
					Err:  err,
